...
- `just check`: run the default full validation suite: lint, Python tests, Electron Node-side tests, and docs build
...
- Electron has an implemented minimal connected updater path, but a real signed/notarized macOS update and Windows NSIS update dry run still need to prove detection, download, restart/install, and `app.sqlite3` persistence before calling it production-ready.
