...
- Add the Electron shell-to-Django auth token pattern: Electron generates a per-session token, passes it as `DESKTOP_DJANGO_AUTH_TOKEN`, injects `X-Desktop-Django-Token` only for the exact local Django origin and health polling, and never exposes it through preload, page JavaScript, query strings, cookies, or `<all_urls>`. Keep CSRF enabled; this token solves a different problem than desktop auto-auth.
