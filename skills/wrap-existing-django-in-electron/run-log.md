...
| 24 | 2026-06-03 | 37m 16s (killed) | fail | — | — | — | — | `805f621` | one-shot · llama.cpp / qwen 3.6 27b served at **only 32k** (the model is **trained for 256k** — llama.cpp warned `n_ctx_seq (32768) < n_ctx_train (262144)`, so this run **under-provisioned** it): authored only 1 partial Django middleware, never started `electron/`; ~24k/32k pressure; stopped (SIGTERM) at 37m to free RAM for ds4. Superseded by the fair-context run 24b below |
| 24b | 2026-06-03 | 79m (ran to completion) | **partial** | — | dev-smoke claimed pass; packaged smoke FAILED | — | 45/45 (node) | `805f621` | one-shot · llama.cpp / qwen 3.6 27b at its **full 256k** context: authored a **near-complete wrap** — 30 model-authored source files / 3,080 insertions (full `electron/` shell + Django integration; `+8` edits; node tests pass). Only the packaged smoke failed: `ModuleNotFoundError: No module named 'core'` (a packaging python-path bug in its settings). PARTIAL — vastly beyond the 32k run; context was the dominant limiter for qwen. Evidence: `.bench-qwen36/results-oneshot-qwen256k/{summary,diff-stat,transcript-summary}.txt,verify-smoke.log` |
| 24c | 2026-06-03 | 72m48s (ran to completion) | fail | — | app `/health/` 500 | — | 45/46 (1 fail) | `805f621` | one-shot · llama.cpp / **Qwen3.6-35B-A3B MoE** (3B active) at full **256k** context — the never-before-run MoE counterpart to 24b: authored *more* (41 source files / 3,343 insertions, 90 bash + 25 write + 9 edit) but **lower quality**. 1 node test failed (electron-builder config packaging gap) and the app is broken at runtime — `/health/` → 500 `AttributeError: 'WSGIRequest' object has no attribute 'user'` (mis-wired auto-auth middleware); also used `smoke:dev` not `smoke:packaged`. **FAIL — worse than the dense 27B's PARTIAL despite ~3–5× faster decode**, consistent with Qwen's own coding benchmarks (dense 27B > 35B-A3B, SkillsBench +15.5). Evidence: `.bench-qwen36/results-oneshot-moe256k/{summary,diff-stat,transcript-summary,own-smoke-evidence}.txt` |
| 24d | 2026-06-03 | 20m (exited, pi_exit=0) | fail | — | — | — | — | `805f621` | one-shot · llama.cpp / qwen 3.6 27b dense at **256k with thinking HIGH** (server `--reasoning on --reasoning-budget -1`; 17 reasoning-budget activations) — the thinking variant of run 24b, never done before. **ANALYSIS-PARALYSIS**: 42 read + 8 bash + **0 writes** over 15 turns, then exited having *quoted* a template in its response instead of writing files. **0 files authored — FAIL, far worse than the same model thinking-OFF (24b: 30 files, PARTIAL).** High reasoning hurt this action-heavy one-shot (mirrors ds4 thinking-high derailing on django-resume). Evidence: `.bench-qwen36/results-oneshot-qwen256k-think/{summary,transcript-summary}.txt` |
| 24e | 2026-06-03 | 114m (ran to completion) | partial | — | packaged smoke FAILED | — | 38/38 (node) | `805f621` | one-shot · llama.cpp / qwen 3.6 27b dense at **256k with thinking MEDIUM** (server `--reasoning-budget 2048`, bounded) — completing the off/medium/high triple. Bounded reasoning **fixed the high-run paralysis**: 137 turns, 43 read + 92 bash + 29 write + 9 edit, 32 source files, node tests 38/38 pass. But packaged smoke FAILED (`KeyError: 'collectstatic'` + `ModuleNotFoundError: No module named 'example'`). **PARTIAL — no better than thinking-OFF (24b), with more packaging bugs and slower (114 vs 79 min).** Net: more reasoning budget did not help qwen's one-shot wrap; off was best, medium ≈ off-but-worse, high collapsed. Evidence: `.bench-qwen36/results-oneshot-qwen256k-med/{summary,diff-stat,transcript-summary,smoke-evidence}.txt` |
| 24f | 2026-06-03 | 67m42s (ran to completion, `agent_end` clean) | partial | — | packaged smoke FAILED (timeout) | — | 51/51 (node) | _uncommitted_ | one-shot · llama.cpp / qwen 3.6 27b dense at **256k, thinking OFF, but at near-lossless Q8_0 (26.6 GB) instead of Q4_K_M (16 GB)** — isolating the **quantization** variable vs run 24b. Outcome **unchanged**: 31 model-authored source files / 3,146 insertions, node tests 51/51 pass, packaged smoke FAILED. The bug *moved* but stayed the same tier — a single self-inconsistent path contract: the model's `stage-backend.cjs` stages `manage.py` at `example/manage.py` while its own `main.js` requires `manage.py` at the **backend root** (`validatePackagedBackendRoot` + `cwd=backendRoot`), so the packaged backend never serves. Verified **not** GPU contention (re-ran smoke with the LLM server stopped → identical timeout). **Q8 did NOT close the gap** (and decoded ~1.7× slower than Q4_K_M); the Q4→Q8 step is where most quant quality is recovered and Q8→bf16 is near-lossless, so bf16 would almost certainly behave the same. Quant is not the bottleneck — one-shot integration reliability is. Evidence: `.bench-qwen36/results-oneshot-qwen-q8-q8/{summary,diff-stat,transcript-summary,verify-smoke-gpufree}.{txt,log}` |
...
| 26 | 2026-06-03 | 25m 29s | fail | — | — | — | — | `805f621` | one-shot · ds4 / DeepSeek V4 Flash @**256k** ctx: no overflow (reached ~132k), 122 bash explorations, then `finish=error "invalid tool call"` while emitting a large settings write — fails on large-file tool-call reliability even with ample context. Conclusion (this benchmark): the frontier control produced a clean PASS in ~7 min; the local results were dominated by **context provisioning** — qwen 3.6 27b at its full 256k window reached a PARTIAL (near-complete wrap, node tests pass, one packaging bug; run 24b), while ds4 failed on 128k overflow and, at 256k, on large-write tool-call reliability. Honest takeaway: serve a local 27B its full trained context and it gets most of the way through a one-shot wrap; a frontier model finishes cleanly and far faster. Evidence from a few local configs on one target — not a universal claim |
